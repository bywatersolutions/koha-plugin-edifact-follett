package Koha::Plugin::Com::ByWaterSolutions::EdifactFollett::Edifact;
...
use Koha::Plugin::Com::ByWaterSolutions::EdifactFollett::Edifact::Segment;
use Koha::Plugin::Com::ByWaterSolutions::EdifactFollett::Edifact::Message;
...
sub message_array {
    ...
    foreach my $seg ( @{ $self->{transmission} } ) {
        if ( $seg->tag eq 'UNH' ) {
            ...
            $in_msg = 0;
            if ( @{$msg} ) {
                push @{$msg_arr}, Koha::Plugin::Com::ByWaterSolutions::EdifactFollett::Edifact::Message->new($msg);
                $msg = [];
            }
        }
        elsif ($in_msg) {
...
(?>         # dont backtrack into this group
    ...
    while ( $raw =~ /($re)/g ) {
        ...
        push @segmented, Koha::Plugin::Com::ByWaterSolutions::EdifactFollett::Edifact::Segment->new( { seg_string => $cap } );
    }
    return \@segmented;
}
